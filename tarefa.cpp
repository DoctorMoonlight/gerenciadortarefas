...
Tarefa::Tarefa(const string& descricao, const string& data, const string& horario, const Etiqueta& etiqueta, const Notificacao& notificacao) : descricao(descricao), data(data), horario(horario), etiqueta(etiqueta), concluida(false), notificacao(notificacao), notificacao_ok(false) {}
...
// Define a tarefa como notificada
void Tarefa::setNotificada(bool notificacao_ok) 
{
    this->notificacao_ok = notificacao_ok;
}

bool Tarefa::isNotificada() const 
{
    return notificacao_ok;
}
